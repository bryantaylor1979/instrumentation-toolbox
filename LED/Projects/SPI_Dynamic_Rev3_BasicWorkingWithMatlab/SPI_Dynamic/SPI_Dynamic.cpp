...
static int	enables[numberOfChannels];
...
//            iparam = atoi(argv[optind]);
			...
			optind++;
			enables[channelNum] = atof(argv[optind]);
			optind++;
...
void printInputs(int *enables, int *currents)
{
	// report enables
	int	i;
	for (i=0;i<numberOfChannels;i++){
		cout << "channelNum = " << i << " Enable = " << enables[i] << " Current = " << currents[i] << endl;
	}
}
